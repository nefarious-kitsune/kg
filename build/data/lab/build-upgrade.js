...
for (let rowIdx = 1; rowIdx < upgradeData.length; rowIdx++) {
  ...
  const lightReagentCost = parseInt(lightCost);
  const darkReagentCost = parseInt(darkCost) || null;

  totalLightReagentCost += lightReagentCost;

  compiledData.data.push(
    {
      'level': level,
      'light-reagent-cost': lightReagentCost,
      'dark-reagent-cost': darkReagentCost,
    }
  )
}
