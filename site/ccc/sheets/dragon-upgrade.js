/* Reference code for calculating Dragonden Emblem upgrades
 *
 * The function takes 3 array parameters:
 * - Current inventory of Rare, Excellent, Perfect, Epic, and Free-Pick Dragon Runes
 * - If Free-Pick Dragon Runes should be exchanged for Rare, Excellent, Perfect, and Epic Dragon Runes
 * - Current Bronze, Silver, Golden, and Legendary Emblem levels
 *
 * The function returns a 2-dimensional array:
 * - The first element is an array for inventory after upgrades
 * - The second element is an array for Emblem levels after upgrades
 * - The third element is an array for Dragon Runes consumed during upgrades
 *
 * Example code
 *   calculateDragonUpgrade(
 *     [20000, 0, 0, 0, 100],
 ...
 * @return {[[Number]]}
 */
function calculateDragonUpgrade(_inventory, _exchange, _emblems) {
  ...
  /**
   * Helper function for parsing integer input
  ...
  if (Array.isArray(_inventory)) {
    ...
    if (_inventory.length >= 5) {
      inventory.rare      = parseInteger(_inventory[0]);
      inventory.excellent = parseInteger(_inventory[1]);
      inventory.perfect   = parseInteger(_inventory[2]);
      inventory.epic      = parseInteger(_inventory[3]);
      inventory.freePick  = parseInteger(_inventory[4]);
    } else {
  ...
  if (Array.isArray(_exchange)) {
    ...
    if (_exchange.length >= 4) {
      exchange.rare      = parseBoolean(_exchange[0]);
      exchange.excellent = parseBoolean(_exchange[1]);
      exchange.perfect   = parseBoolean(_exchange[2]);
      exchange.epic      = parseBoolean(_exchange[3]);
    } else {
  ...
  if (Array.isArray(_emblems)) {
    ...
    if (_exchange.length >= 4) {
      emblems.bronze    = parseInteger(_emblems[0]);
      emblems.silver    = parseInteger(_emblems[1]);
      emblems.golden    = parseInteger(_emblems[2]);
      emblems.legendary = parseInteger(_emblems[3]);
    } else {
